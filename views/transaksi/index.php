                                        ...
                                        <div class="trx-actions d-flex align-items-center justify-content-center gap-1">
                                            <?php if (($transaksi['status'] ?? 'diverifikasi') === 'diajukan'): ?>
                                                ...
                                                <span class="trx-more-pane">
                                                    ...
                                                    <button type="button"
                                                            class="btn-action btn-action-print-direct btn-cetak-langsung"
                                                            data-id="<?= $transaksi['id'] ?>"
                                                            data-bukti="<?= htmlspecialchars($transaksi['nomor_bukti'] ?? '') ?>"
                                                            data-bs-toggle="tooltip"
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function showPrintToast(message, type) {
        type = type || 'success';
        var toastEl = document.getElementById('printToastAdmin');
        if (toastEl && typeof bootstrap !== 'undefined') {
            var iconEl = document.getElementById('printToastIconAdmin');
            var msgEl = document.getElementById('printToastMessageAdmin');
    ...
    // Handler Tombol Cetak Langsung via AJAX (Admin)
    document.querySelectorAll('.btn-cetak-langsung').forEach(function(btn) {
        btn.addEventListener('click', function(e) {
            ...
            self.disabled = true;
            self.innerHTML = '<span class="spinner-border spinner-border-sm" role="status" aria-hidden="true" style="width:12px;height:12px;"></span>';

            var baseUrlStr = (typeof VERIF_BASE !== 'undefined' ? VERIF_BASE : '<?= rtrim(base_url(), "/") ?>') + '/';
...
<!-- Toast Container untuk Notifikasi Ajax Cetak Langsung Admin -->
<div class="toast-container position-fixed top-0 end-0 p-3" style="z-index: 1090;">
    <div id="printToastAdmin" class="toast align-items-center border-0 shadow-lg" role="alert" aria-live="assertive" aria-atomic="true">
        <div class="d-flex">
            <div class="toast-body d-flex align-items-center gap-2">
                <i id="printToastIconAdmin" class="bi fs-5"></i>
                <span id="printToastMessageAdmin"></span>
            </div>
